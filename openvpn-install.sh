...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
    ...
    ip_number="1"  # Set the default to the first IPv4 address
    ...
    ip6_number="1"  # Set the default to the first IPv6 address
    ip6=$(ip -6 addr | grep 'inet6 [23]' | cut -d '/' -f 1 | grep -oE '([0-9a-fA-F]{0,4}:){1,7}[0-9a-fA-F]{0,4}' | sed -n "$ip6_number"p)
    fi
	protocol=udp
	echo "Protocol is set to UDP"

	echo "OpenVPN is listening to port 443"
	port=443

	echo

	echo "DNS server for the clients: 1.1.1.1"
	dns=3
